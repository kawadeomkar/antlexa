...
def get_welcome_response():
    ...
    card_title = 'Welcome'
    speech_output = \
        'Welcome to Antlexa Express, please say a bus stop I.D. to get the bus times.'
...
def get_bus_times_in_session(intent, session):
    ...
    if 'stops' in intent['slots']:
        numTimes = len(intent['slots']['stops']['resolutions'
    ...
    # returned more than one stop id, must exit and ask user for more
        ...
        if numTimes > 1:
            speech_output = \
                'I got more than one stop I.D. please try again.'
            reprompt_text = \
                "I'm getting more than one stop I.D. Please try again or check the documentation for available bus stops."
        else:
        ...
        speech_output = \
            "I'm not sure what bus stop that is. Please try again."
        reprompt_text = \
            "I'm not sure what bus stop you are trying to say. You can look at the available bus stops in the documentation."
    return build_response(session_attributes,
                          build_speechlet_response(card_title,
                          speech_output, reprompt_text,
                          should_end_session))
